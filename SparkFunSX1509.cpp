...
September 16, 2016
Rick Koch / N1GP, modified for Linux SBC uses.
...
byte SX1509::readByte(byte registerAddress)
{
	...
	readValue = wiringPiI2CReadReg8(deviceFd, registerAddress);
...
unsigned int SX1509::readWord(byte registerAddress)
{
	...
	readValue = wiringPiI2CReadReg16(deviceFd, registerAddress);
...
void SX1509::readBytes(byte firstRegisterAddress, byte * destination, byte length)
{
	...
	{
		destination[i] = wiringPiI2CReadReg8(deviceFd, firstRegisterAddress + i);
	}
}
...
{
	wiringPiI2CWriteReg8(deviceFd, registerAddress, writeValue);
}
...
void SX1509::writeWord(byte registerAddress, unsigned int writeValue)
{
	...
	wiringPiI2CWriteReg16(deviceFd, registerAddress, writeWord);
}
...
void SX1509::writeBytes(byte firstRegisterAddress, byte * writeArray, byte length)
{
	for (int i=0; i<length; i++)
	{
		wiringPiI2CWriteReg8(deviceFd, firstRegisterAddress + i, writeArray[i]);
	}
}
